...
import com.force.codes.mynotes.core.widget.swipeadater.util.DragDropSwipeItemDecoration
...
open class DragDropSwipeRecyclerView @JvmOverloads constructor(
    ...
    override fun onRestoreInstanceState(state: Parcelable?) {
        ...
        if (isSaveEnabled && state is Bundle) {
            ...
            behindSwipedItemIconSecondaryDrawableId = state.getInt(
                BEHIND_SWIPED_ITEM_ICON_SECONDARY_DRAWABLE_ID_KEY, 0)
            ...
            behindSwipedItemBackgroundSecondaryColor = state.getInt(
                BEHIND_SWIPED_ITEM_BACKGROUND_SECONDARY_COLOR_KEY, Color.TRANSPARENT)
            behindSwipedItemLayoutId = state.getInt(BEHIND_SWIPED_ITEM_LAYOUT_ID_KEY, 0)
            behindSwipedItemSecondaryLayoutId = state.getInt(
                BEHIND_SWIPED_ITEM_SECONDARY_LAYOUT_ID_KEY, 0)
